...
const InvitadosView = ({ tables, guests, setGuests, addNotification, tipoEvento }) => {
  ...
  // 🔴 SOLUCIÓN ANTI-POPUPS: Abrir WhatsApp de inmediato
  ...
  // 🔴 LÓGICA DE ACOMPAÑANTES EN BLANCO Y SUMA CORRECTA
  ...
  // (PDF functions omitted from internal rendering logic but kept structure intact)
  ...
  return (
    <div className="h-full flex flex-col space-y-6 pb-6 relative text-slate-900 dark:text-slate-200 transition-colors duration-500">
      ...
      {/* 🔴 MODAL QR FALTANTE AGREGADO AQUÍ */}
      {qrModal && (
        <div className="fixed inset-0 z-[200] bg-slate-900/80 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in transition-colors">
...
const LandingPageView = ({ isDarkMode, themeSetting, cycleTheme }) => {
  ...
  const handlePaymentSuccess = (datosCliente) => {
    // 🔴 Aquí en Fase 6 dispararemos la Cloud Function para crear la base de datos real
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] dark:bg-[#050505] font-sans text-slate-900 dark:text-slate-100 selection:bg-amber-500 selection:text-white transition-colors duration-700 overflow-x-hidden">
      ...
      {/* RESTO DE LA LANDING (Sin cambios en este bloque, lo mantenemos igual) */}
      <style>{`
...
const AdminDashboard = ({ authData, cycleTheme, themeSetting, isDarkMode }) => {
  ...
  const renderContent = () => {
    switch(activeTab) {
      ...
      case 'invitados': return typeof InvitadosView !== 'undefined' ? <InvitadosView tables={tables} guests={guests} setGuests={setGuests} addNotification={addNotification} tipoEvento={currentEvent?.tipoEvento} /> : null; 
